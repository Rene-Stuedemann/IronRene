...
select
account_id,
(case when type in ("PRIJEM") then "Incoming" when type in ("VYDAJ") then "Outgoing" end) as transaction_type,
...
select
account_id,
round(sum(case when type in ("PRIJEM") then amount end)) as Incoming,
round(sum(case when type in ("VYDAJ") then amount end)) as Outgoing,
round(sum(case when type in ("PRIJEM") then amount end))-round(sum(case when type in ("VYDAJ") then amount end)) as Difference
from bank.trans
where account_id = 396
group by account_id;
